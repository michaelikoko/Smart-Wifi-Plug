...
def _save_telemetry_to_db(payload: TelemetryPayload, device_id: str, session: SessionDep):
    """
    ...
    try:
        ...
        current_summary = session.exec(
            select(DeviceDailySummary).where(
                DeviceDailySummary.device_id == device_id). where(DeviceDailySummary.date == date)
            ).first()                       
        ...
        if current_summary is None:
            ...
            current_summary = DeviceDailySummary(
                ...
                energy_first_timestamp=datetime.fromtimestamp(resolved_ts_epoch, tz=timezone.utc),
                energy_last=payload.e,
                energy_last_timestamp=datetime.fromtimestamp(resolved_ts_epoch, tz=timezone.utc),
                kwh_consumed=0.0,
                peak_power=payload.p,
                peak_power_timestamp=datetime.fromtimestamp(resolved_ts_epoch, tz=timezone.utc),
            )
            session.add(current_summary)
            logger.info(
                "Day baseline set — device=%s date=%s energy_first=%.3f kWh",
                device_id, date, payload.e
            )
        else:
            # Update existing summary
            current_summary.energy_last = payload.e
            current_summary.energy_last_timestamp = datetime.fromtimestamp(resolved_ts_epoch, tz=timezone.utc)
            current_summary.kwh_consumed = round(current_summary.energy_last - current_summary.energy_first, 4)

            if payload.p > (current_summary.peak_power or 0):
                current_summary.peak_power = payload.p
                current_summary.peak_power_timestamp = datetime.fromtimestamp(resolved_ts_epoch, tz=timezone.utc)
        ...
        if device:
            ...
            logger.info(
                ...
                device_id, device.last_seen, device.relay_state,
            )
        else:
            logger.warning(
                "Telemetry received for unregistered device: %s", device_id
            )
        session.commit()
...
def register_mqtt_handlers():
    ...
    async def on_status(
        ...
        logger.info("Status — topic=%s payload=%s",
        ...
        try:
            with Session(engine) as session:
                ...
                if device:
                    ...
                    device.is_online = True # Device is online if we're receiving relay state updates
                    device.last_seen = datetime.now(timezone.utc) 
                    session.add(device)
                    session.commit()
                    logger.info(
                        "Relay state updated — device=%s state=%s",
                        device_id, state
                    )
                else:
                    logger.warning(
                        "Relay state received for unregistered device: %s", device_id
                    )
        except Exception as e:
...
async def start_staleness_sweep():
    ...
    logger.info(
        ...
        STALENESS_THRESHOLD_S, STALENESS_SWEEP_INTERVAL_S,
    )
    while True:
