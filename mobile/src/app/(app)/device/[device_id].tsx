...
import {
    ...
    Battery, CircleDot,
    Clock,
    Gauge, Pencil, Radio, ToggleLeft, ToggleRight, Trash2, Wifi, Zap,
...
import { deleteDevice, listDevices, updateDeviceLimits } from '../../../api/devices-api';
import { getEnergyHistory, is404 } from '../../../api/telemetry-api';
import { createTimer, deleteTimer, listTimers, rearmTimerLock, updateTimer, type TimerResponse } from '../../../api/timers-api';
import { publishRelayCommand, publishWifiChangeCommand, unsubscribeFromDevice } from '../../../lib/mqtt-client';
...
export default function DeviceDetailScreen() {
    ...
    const rearmMutation = useMutation({
        ...
        onSuccess: async () => {
            // The retained be-timer-lock MQTT message (locked: false) will update
            // the live store on its own — this invalidate is just a safety net
            // for the REST-cached device list (e.g. cutoff_reason display elsewhere).
            await queryClient.invalidateQueries({ queryKey: ['devices'] });
        },
    });
                    ...
                    <VStack className="gap-3">
                        ...
                        <Pressable
                            onPress={async () => {
                                ...
                                const res = await publishWifiChangeCommand(device_id, wifiSsid.trim(), wifiPassword);
